...
async function run() {
  log('========================================');
  log('STARTING OPTIMIZED BLOOD BANK MERGE');
  log('========================================');
  ...
  try {
    if (!fs.existsSync(CLEANED_DIR)) {
      log(`ERROR: Cleaned blood bank directory ${CLEANED_DIR} not found.`);
      process.exit(1);
    }

    const files = fs.readdirSync(CLEANED_DIR).filter(file => file.endsWith('.json'));
    log(`Found ${files.length} cleaned blood bank files to merge.`);
    ...
    for (const record of allRecords) {
      ...
      for (let j = 0; j < group.length; j++) {
        ...
        if (record.phone && master.phone && record.phone === master.phone && record.phone.replace(/[^\d]/g, '').length >= 10) {
          matchIndex = j;
          dupLog(`[PHONE MATCH BB] "${record.name}" with master "${master.name}" (Phone: ${record.phone})`);
          break;
        }
        ...
        if (record.location?.coordinates && master.location?.coordinates) {
          ...
          if (dist < 0.150) {
            ...
            if (sim >= 0.50) {
              matchIndex = j;
              dupLog(`[DISTANCE MATCH BB] Dist: ${Math.round(dist * 1000)}m, Sim: ${Math.round(sim * 100)}% | "${record.name}" with "${master.name}"`);
              break;
            }
          }
        }
        ...
        if (sim >= 0.85) {
          matchIndex = j;
          dupLog(`[NAME SIMILARITY MATCH BB] Name Sim: ${Math.round(sim * 100)}% | "${record.name}" with "${master.name}" in city "${cityVal}"`);
          break;
        }
      }
    ...
    log(`Consolidation results for Blood Banks:`);
    ...
    const masterOutputPath = path.join(MERGED_DIR, 'bloodbanks_Master.json');
    fs.writeFileSync(masterOutputPath, JSON.stringify(masterRecords, null, 2), 'utf-8');
    log(`SUCCESS: Master consolidated blood bank file written to: ${masterOutputPath}`);
  } catch (error) {
    log(`CRITICAL ERROR during blood bank merge: ${error.stack}`);
    process.exit(1);
  }
}
