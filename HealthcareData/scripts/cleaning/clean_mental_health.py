...
def clean_mental_health():
    plog = PipelineLogger("Clean_Mental_Health", "clean_mental_health.log")
    plog.info("Starting clean mental health dataset pipeline...")
    
    input_path = os.path.join(RAW_DIR, "mental_health", "Mental Health Dataset.csv")
    output_dir = os.path.join(CLEANED_DIR, "mental_health")
    os.makedirs(output_dir, exist_ok=True)
    output_path = os.path.join(output_dir, "mental_health.csv")
    ...
    try:
        ...
        # Validate
        validate(df, "mental_health", plog)
        ...
        plog.error(f"Error during mental health cleaning: {str(e)}")
