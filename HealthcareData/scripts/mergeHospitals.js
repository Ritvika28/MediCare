...
function mergeRecords(rec1, rec2) {
  ...
  // Merge list tags
  const facilities = [...new Set([...(rec1.facilities || []), ...(rec2.facilities || [])])];
  const specialties = [...new Set([...(rec1.specialties || []), ...(rec2.specialties || [])])];
  ...
  return {
    ...
    facilities,
    specialties,
    totalBeds: Math.max(rec1.totalBeds || 0, rec2.totalBeds || 0),
...
async function run() {
  log('========================================');
  log('STARTING OPTIMIZED HOSPITAL MERGE');
  log('========================================');
  ...
  try {
    if (!fs.existsSync(CLEANED_DIR)) {
      log(`ERROR: Cleaned directory ${CLEANED_DIR} not found.`);
      process.exit(1);
    }

    const files = fs.readdirSync(CLEANED_DIR).filter(file => file.endsWith('.json'));
    log(`Found ${files.length} cleaned files to merge.`);
    ...
    for (const record of allRecords) {
      ...
      for (let j = 0; j < group.length; j++) {
        ...
        if (record.phone && master.phone && record.phone === master.phone && record.phone.replace(/[^\d]/g, '').length >= 10) {
          matchIndex = j;
          dupLog(`[PHONE MATCH] "${record.name}" with master "${master.name}" (Phone: ${record.phone})`);
          break;
        }

        // 2. Coordinate match (only if distance < 150m AND name similarity >= 0.5)
        if (record.location?.coordinates && master.location?.coordinates) {
          ...
          if (dist < 0.150) { // 150 meters
            ...
            if (sim >= 0.50) {
              matchIndex = j;
              dupLog(`[DISTANCE & NAME MATCH] Distance: ${Math.round(dist * 1000)}m, Name Sim: ${Math.round(sim * 100)}% | "${record.name}" with "${master.name}"`);
              break;
            }
          }
        }

        // 3. Name similarity match (similarity >= 0.85)
        const sim = stringSimilarity(record.name, master.name);
        if (sim >= 0.85) {
          matchIndex = j;
          dupLog(`[NAME SIMILARITY MATCH] Name Sim: ${Math.round(sim * 100)}% | "${record.name}" with "${master.name}" in city "${cityVal}"`);
          break;
        }
      }
    ...
    const masterOutputPath = path.join(MERGED_DIR, 'hospitals_Master.json');
    fs.writeFileSync(masterOutputPath, JSON.stringify(masterRecords, null, 2), 'utf-8');
    log(`SUCCESS: Master consolidated hospital file written to: ${masterOutputPath}`);
  } catch (error) {
    log(`CRITICAL ERROR during hospital merge: ${error.stack}`);
    process.exit(1);
  }
}
